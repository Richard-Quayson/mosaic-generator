    ...
    for line in extracted_text.split("\n"):
        if not start:
            ...
            if start:
                ...
                quadrant_line_count = (
                    0  # keep track of the number of lines read per quadrant
                )
        else:
            ...
            if len(line_content) == 1:
                ...
                if block_letter != current_block_letter:
                    ...
                    block_list = create_row_colour_list(
                        sub_quadrant_one,
                        sub_quadrant_two,
                        sub_quadrant_three,
                ...
                if current_block_letter.upper() == BLOCK_WITH_MIN_SUB_QUADRANTS:
                    if (quadrant_line_count % MIN_NUM_SUB_QUADRANT_ITEMS_PER_ROW) < (
                        dimension
                    ):
                    ...
                    if (quadrant_line_count % MAX_NUM_SUB_QUADRANT_ITEMS_PER_ROW) < (
                        dimension
                    ):
                        sub_quadrant_one.append(line_content)
                    elif (quadrant_line_count % MAX_NUM_SUB_QUADRANT_ITEMS_PER_ROW) < (
                        2 * dimension
                    ):
                        sub_quadrant_two.append(line_content)
                    else:
...
def create_colour_list_file(colour_list: list, file_path: str) -> tuple[str, str]:
    """
    ...
    Returns:
        - tuple: the path to the created .py colour list file and the variable name
    """

    output_filepath = file_path.replace(".pdf", ".py")
    variable_name = (
        file_path.split("/")[-1].replace(".pdf", "").upper() + "_COLOUR_LIST"
    )
    ...
    return output_filepath, variable_name


def main(file_path: str, dimension: int) -> tuple[str, str]:
    """
    ...
    colour_list = extract_colour_list(extracted_text, dimension)

    # create colour list .py file
    colour_list_file, colour_list_name = create_colour_list_file(colour_list, file_path)

    # return the colour list file
    return colour_list_file, colour_list_name
